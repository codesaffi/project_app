      ...
      {posts.map((post) => (
        <div key={post._id} className="post-container">
          <div className="post-header">
            <img className="post-profile-picture" src={`http://localhost:5000/${profile.profilePicture}`} alt="Profile" />
            <span className="post-username">{profile.username}</span>
          </div>
          <img className="post-image" src={`http://localhost:5000/${post.image}`} alt="Post" />
