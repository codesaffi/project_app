...
const FriendProfile = () => {
  ...
  useEffect(() => {
    const fetchFriendProfile = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/users/profile/${id}/posts`, {
  ...
  return (
    <>
      ...
      {friendProfile.coverPicture && (
        ...
        <img 
          src={`http://localhost:5000/${friendProfile.coverPicture}`} 
          alt="Cover" 
          className="user-cover-picture" 
        />
      )}
      <img 
        src={`http://localhost:5000/${friendProfile.profilePicture}`} 
